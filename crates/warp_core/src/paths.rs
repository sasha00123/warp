...
fn base_warp_config_dir_name() -> String {
    match ChannelState::channel() {
        ...
        Channel::Oss => format!("{WARP_CONFIG_DIR}-custom"),
...
/// `WarpCustom` in this distribution.
#[cfg(any(not(target_os = "macos"), test))]
fn gui_app_id_for_channel(channel: Channel, current_app_id: AppId) -> AppId {
    match channel {
        Channel::Oss => AppId::new("io", "sasha00123", "WarpCustom"),
        Channel::Stable
        | Channel::Preview
        | Channel::Dev
        | Channel::Integration
...
pub fn secure_state_dir() -> Option<PathBuf> {
    // Integration tests use a temporary home. The OSS distribution must not use
    // the official application's Apple Team App Group container.
