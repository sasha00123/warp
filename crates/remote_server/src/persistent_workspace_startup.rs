...
fn failure(error: std::io::Error) -> Error {
    Error::new(ErrorKind::Failed, format!("Persistent shell startup: {error}"))
}
...
impl Backend {
    ...
    pub fn create_initialized(
        &self, id: &str, directory: Option<&Path>, profile: BootstrapProfile<'_>,
        executable: &Path, root: &Path,
    ) -> Result<Workspace, Error> {
        self.create_initialized_with_retention(id, directory, profile, executable, root,
            crate::persistent_journal::RetentionPolicy::default())
    }

    pub fn create_initialized_with_retention(
        &self, id: &str, directory: Option<&Path>, profile: BootstrapProfile<'_>,
        executable: &Path, root: &Path, retention: crate::persistent_journal::RetentionPolicy,
    ) -> Result<Workspace, Error> {
        validate_token(id)?;
        if !persistent_shell::supported_shell(profile.shell)
            || profile.init_script.is_empty() || profile.init_script.len() > 64 * 1024
            || profile.bootstrap_script.is_empty() || profile.bootstrap_script.len() > 1024 * 1024
            || !root.is_absolute() || root.to_str().is_none()
            || !executable.is_absolute() || executable.to_str().is_none()
        { return Err(Error::new(ErrorKind::InvalidRequest, "Invalid persistent shell profile")); }
        ...
        if !init.contains("@@WARP_SESSION_ID@@") {
            return Err(Error::new(ErrorKind::InvalidRequest, "Shell init is missing its session token placeholder"));
        }
        fs::DirBuilder::new().recursive(true).mode(0o700).create(root).map_err(failure)?;
        let metadata = fs::symlink_metadata(root).map_err(failure)?;
        if !metadata.is_dir() || metadata.permissions().mode() & 0o077 != 0 {
            return Err(Error::new(ErrorKind::Failed, "Shell state root must be private (0700)"));
        }
        ...
        let lock = OpenOptions::new().create(true).truncate(false).read(true).write(true).mode(0o600)
            .open(root.join(format!("{id}.lock"))).map_err(failure)?;
        let deadline = Instant::now() + self.timeout;
        loop {
            match lock.try_lock() {
                Ok(()) => break,
                Err(TryLockError::WouldBlock) if Instant::now() < deadline => thread::sleep(Duration::from_millis(10)),
                Err(TryLockError::WouldBlock) => return Err(Error::new(ErrorKind::Timeout, "Workspace initialization is still in progress")),
        ...
        match self.lookup(id) {
            Ok(workspace) => {
                ...
                if !startup.join("startup").is_file() {
                    return Err(Error::new(ErrorKind::Failed, "Existing workspace was not created with durable shell integration"));
                }
        ...
        fs::DirBuilder::new().mode(0o700).create(&startup).map_err(failure)?;
        let session_id = persistent_shell::session_id(&generation).unwrap();
        let init = init.replace("@@WARP_SESSION_ID@@", &session_id.to_string())
        ...
        let prefix = if profile.shell == "fish" {
            format!("set -gx WARP_PERSISTENT_ACTIVITY_PATH {activity_path}\nset -gx WARP_PERSISTENT_ROOT_SESSION {session_id}\n")
        } else {
            format!("export WARP_PERSISTENT_ACTIVITY_PATH={activity_path}\nexport WARP_PERSISTENT_ROOT_SESSION={session_id}\n")
        };
        ...
        private_write(&startup.join("retention"), match retention {
        ...
        let shell = shell.to_str().ok_or_else(|| Error::new(ErrorKind::Failed, "Shell path is not UTF-8"))?;
        let command = format!("exec {} {} {} {} {}", quote(executable.to_str().unwrap()),
            persistent_shell::SHELL_ARGUMENT, quote(startup.to_str().unwrap()), quote(profile.shell), quote(shell));
        let workspace = self.create_with_startup(id, directory, Some(&generation), Some(&command))?;
        self.finish_startup(&workspace, executable, root, &startup)?;
        Ok(workspace)
    }

    fn finish_startup(&self, workspace: &Workspace, executable: &Path, root: &Path, startup: &Path) -> Result<(), Error> {
        // Never rerun initialization after release, even if a job is now running.
        if startup.join("ready").is_file() { return Ok(()); }
        let retention = match fs::read(startup.join("retention")) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => crate::persistent_journal::RetentionPolicy::default(),
            Ok(_) => return Err(Error::new(ErrorKind::Failed, "Invalid saved history retention policy")),
            Err(error) => return Err(failure(error)),
        };
        self.start_recording_with_retention(&workspace.id, &workspace.generation, executable, root, retention)?;
        private_write(&startup.join("ready"), b"ready\n")?;
        File::open(startup).and_then(|file| file.sync_all()).map_err(failure)
    }
}
