...
pub fn find_shell(shell: &str) -> io::Result<PathBuf> {
    if !supported_shell(shell) {
        return Err(io::Error::new(
            ...
            "Unsupported persistent shell",
        ));
    }
    let path = std::env::var_os("PATH").unwrap_or_default();
    for directory in
        std::env::split_paths(&path).chain([PathBuf::from("/bin"), PathBuf::from("/usr/bin")])
    {
    ...
    Err(io::Error::new(
        ...
        format!("Remote shell {shell} is not installed"),
    ))
}

pub fn run_shell_if_requested() -> Option<io::Result<()>> {
    let mut args = std::env::args_os().skip(1);
    if args.next().as_deref() != Some(std::ffi::OsStr::new(SHELL_ARGUMENT)) {
        return None;
    }
    Some((|| {
        let invalid = || {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid persistent shell launch",
            )
        };
        ...
        command
            ...
            .env_remove("WARP_BOOTSTRAPPED")
            .env_remove("WARP_SESSION_ID")
            .env_remove("BASH_ENV")
            .env_remove("ENV");
        match shell.as_str() {
            "bash" => {
                command
                    ...
                    .arg(directory.join("startup"))
                    .arg("-i");
            }
            "zsh" => {
                if let Some(original) = std::env::var_os("ZDOTDIR") {
                    command
                        .env("WARP_PERSISTENT_ORIGINAL_ZDOTDIR", original)
                        .env("WARP_PERSISTENT_HAD_ZDOTDIR", "1");
                } else {
                    command
                        .env_remove("WARP_PERSISTENT_ORIGINAL_ZDOTDIR")
            ...
            "fish" => {
                ...
                command
                    .args(["--no-config", "--interactive", "--init-command"])
