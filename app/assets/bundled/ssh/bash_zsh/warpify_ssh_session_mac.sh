...
_warp_tmux_target_exists() {
    ...
    _window_id="$2"
    _pane_id="$3"
    [ -n "$_window_id" ] && [ -n "$_pane_id" ] && $TMUX -Lwarp list-panes -s -t "$_session_name" -F '#{window_id} #{pane_id}' 2>/dev/null | command -p grep -Fx "$_window_id $_pane_id" >/dev/null 2>&1
}
...
_warp_tmux_create_target() {
    ...
    if $TMUX -Lwarp has-session -t "$_session_name" 2>/dev/null; then
        $TMUX -Lwarp set-environment -u -t "$_session_name" WARP_BOOTSTRAPPED 2>/dev/null || true
        $TMUX -Lwarp set-environment -u -t "$_session_name" WARP_BOOTSTRAP_VAR 2>/dev/null || true
        $TMUX -Lwarp new-window -d -P -F '#{window_id} #{pane_id}' -t "$_session_name:" -n "$_window_name"
    else
        $TMUX -Lwarp new-session -d -P -F '#{window_id} #{pane_id}' -s "$_session_name" -n "$_window_name"
    fi
}
...
_warp_tmux_print_picker() {
    ...
    $TMUX -Lwarp list-panes -s -t "$_session_name" -F '#{pane_id}|#{window_id}|#{window_name}|#{pane_current_command}|#{pane_current_path}|#{pane_active}' > "$_target_file" 2>/dev/null || true
...
_warp_tmux_attach() {
    ...
    $TMUX -Lwarp set-option -t "$_session_name" destroy-unattached off 2>/dev/null || true
    $TMUX -Lwarp set-option -t "$_session_name" allow-passthrough on 2>/dev/null || true
    $TMUX -Lwarp set-environment -u -t "$_session_name" WARP_BOOTSTRAPPED 2>/dev/null || true
    $TMUX -Lwarp select-window -t "$_window_id" 2>/dev/null || true
    $TMUX -Lwarp select-pane -t "$_pane_id" 2>/dev/null || true
    _warp_tmux_save_target "$_session_name" "$_window_id" "$_pane_id"
    $TMUX -Lwarp -CC attach-session -t "$_session_name"
}
