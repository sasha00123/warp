...
fn page(start: u64, bytes: &[u8]) -> PersistentTerminalOutput {
    PersistentTerminalOutput { start_cursor: start, next_cursor: start + bytes.len() as u64,
        earliest_cursor: 0, high_watermark: start + bytes.len() as u64,
        history_gap: false, closed: false, output: bytes.to_vec(), recorder_connected: true }
}
...
fn persistent_replay_suppresses_historical_queries_but_answers_live_queries() {
    ...
    replay.connected();
    let query = b"\x1b[c";
    let replies = replay.apply(replay.read_ticket().unwrap(), page(0, query)).unwrap();
    assert!(replies.is_empty());
    assert!(matches!(events.try_recv().unwrap(), Event::PersistentReplayState { replaying: true }));
    assert!(matches!(events.try_recv().unwrap(), Event::PersistentReplayState { replaying: false }));
    assert_eq!(replay.phase(), ReplayPhase::Live);
    let replies = replay.apply(replay.read_ticket().unwrap(), page(query.len() as u64, query)).unwrap();
    assert!(!replies.is_empty(), "Live terminal queries must still be answered");
}
...
fn persistent_replay_rejects_a_previous_connection_before_mutating_the_model() {
    ...
    replay.connected();
    assert_eq!(replay.apply(old, page(0, b"stale output")), Err(ReplayError::StaleResponse));
...
fn persistent_replay_does_not_advance_past_a_retention_gap() {
    ...
    assert_eq!(replay.apply(replay.read_ticket().unwrap(), retained),
        Err(ReplayError::HistoryGap { earliest_cursor: 10 }));
    assert_eq!(replay.cursor(), 0);
