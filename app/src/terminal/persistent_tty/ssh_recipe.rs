...
fn parse_arguments(arguments: &[String]) -> Result<ParsedArguments> {
    ...
    while index < arguments.len() {
        ...
        if !argument.starts_with('-') || argument == "-" { break; }
        for (offset, flag) in argument[1..].char_indices() {
            ...
            if "BbcDEeFIiJLlmopRS".contains(flag) {
                ...
                let value = if value_offset < argument.len() {
                    ...
                    index += 1;
                    arguments.get(index).cloned().ok_or_else(|| anyhow!("SSH -{flag} needs an argument"))?
                };
                if value.is_empty() { bail!("SSH -{flag} has an empty argument"); }
    ...
    Ok(ParsedArguments { options, destination })
}
...
impl SshRecipe {
    ...
    fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let fields = bytes[..bytes.len() - 1].split(|byte| *byte == 0)
            .map(std::str::from_utf8).collect::<std::result::Result<Vec<_>, _>>()?;
        if fields.len() < 3 || fields[0] != "EWSSH1" { bail!("Unknown SSH reconnect record format"); }
        let recipe = Self { working_directory: PathBuf::from(fields[1]),
            arguments: fields[2..].iter().map(|field| (*field).to_owned()).collect() };
        recipe.validate()?;
        Ok(recipe)
    }
    ...
    pub fn recovery_command(&self, id: &str, generation: &str) -> Result<String> {
        ...
        for setting in ["ControlMaster=no", "ControlPersist=no", "ControlPath=none",
            "StrictHostKeyChecking=yes", "ClearAllForwardings=yes", "RequestTTY=force",
            "SessionType=default", "RemoteCommand=none", "ServerAliveInterval=10", "ServerAliveCountMax=2"] {
            args.extend(["-o".into(), setting.into()]);
        }
        for (flag, value) in parsed.options {
            if "SMNnTtDLR".contains(flag) { continue; }
            if flag == 'o' && let Some(value) = &value {
                let key = value.split(['=', ' ', '\t']).next().unwrap_or_default();
                if ["ControlMaster", "ControlPath", "ControlPersist", "RequestTTY", "SessionType",
                    "LocalForward", "RemoteForward", "DynamicForward", "ForkAfterAuthentication",
                    "RemoteCommand", "StdinNull"].iter().any(|item| key.eq_ignore_ascii_case(item)) { continue; }
            }
        ...
        let attach = format!("set-option -t {session} status off ; set-option -w -t ={session}:0 window-size latest ; attach-session -t {session}");
        let remote = ["tmux", "-L", remote_server::persistent_workspace::SOCKET, "if-shell", "-F",
            "-t", &target, &guard, &attach, "display-message -p 'Workspace was removed or replaced; nothing was started'"]
            .into_iter().map(quote).collect::<Vec<_>>().join(" ");
        args.extend(["--".into(), parsed.destination, remote]);
        let directory = self.working_directory.to_str().ok_or_else(|| anyhow!("SSH working directory is not UTF-8"))?;
        Ok(format!("cd -- {} && /usr/bin/ssh {}", quote(directory),
    ...
    pub fn master_arguments(&self, socket: &Path) -> Result<Vec<String>> {
        self.validate()?;
        let socket = socket.to_str().filter(|_| socket.is_absolute())
            .ok_or_else(|| anyhow!("SSH control socket must be an absolute UTF-8 path"))?;
        let parsed = parse_arguments(&self.arguments)?;
        let mut args = vec!["-N".into(), "-T".into()];
        for setting in [
            "ControlMaster=yes", "ControlPersist=no", "BatchMode=yes",
            "StrictHostKeyChecking=yes", "ServerAliveInterval=10", "ServerAliveCountMax=2",
            "ClearAllForwardings=yes", "RequestTTY=no", "SessionType=none", "ExitOnForwardFailure=yes",
        ] { args.extend(["-o".into(), setting.into()]); }
        ...
        for (flag, value) in parsed.options {
            ...
            if "SMNTtDLR".contains(flag) { continue; }
            if flag == 'o' && let Some(value) = &value {
                let key = value.split(['=', ' ', '\t']).next().unwrap_or_default();
                if ["ControlMaster", "ControlPath", "ControlPersist", "RequestTTY", "SessionType",
                    "LocalForward", "RemoteForward", "DynamicForward", "ForkAfterAuthentication",
                    "RemoteCommand"].iter().any(|item| key.eq_ignore_ascii_case(item)) { continue; }
            }
            args.push(format!("-{flag}"));
            if let Some(value) = value { args.push(value); }
        }
