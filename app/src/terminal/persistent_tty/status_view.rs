...
use warpui::elements::{ChildAnchor, ChildView, Container, Flex, Hoverable, MouseStateHandle,
...
use crate::context_chips::persistent_workspace_popup::{PersistentWorkspacePopup, PickerConnection, WorkspaceKey};

#[derive(Clone, Debug)]
pub enum Action { AcknowledgeInput, OpenWorkspaces, RetryOutput, OpenRecovery }

#[derive(Clone, Debug, PartialEq, Eq)]
struct StatusText { message: String, acknowledge: bool, retry: bool, recover: bool }

fn status_text(transport: &TransportStatus, connection: &ConnectionStatus, running: bool) -> StatusText {
    let message = match &transport.phase {
        TransportPhase::Live => if running { "Running | persistent SSH" } else { "Idle | persistent SSH" }.into(),
        ...
        TransportPhase::Removed => "Workspace was removed or replaced | no new job was created".into(),
        ...
        TransportPhase::Disconnected => match connection {
            ConnectionStatus::Connecting => "Connecting | remote job is independent of this tab".into(),
    ...
    let message = if let Some(bytes) = transport.history_storage_bytes
        ...
        format!("{message}\nRetained history uses {:.1} GiB on the remote host. Delete unused workspaces to free space.", bytes as f64 / (1024.0 * 1024.0 * 1024.0))
    } else { message };
    ...
    StatusText { message, acknowledge, retry: matches!(transport.phase, TransportPhase::Failed(_)),
        recover: matches!(transport.phase, TransportPhase::Failed(_) | TransportPhase::HistoryGap { .. }) }
}
...
impl StatusView {
    pub fn new(transport: TransportHandle, owner: ModelHandle<ConnectionOwner>,
        model: Arc<FairMutex<TerminalModel>>, terminal_view_id: EntityId, ctx: &mut ViewContext<Self>) -> Self {
        let status = status_text(&transport.status(), owner.as_ref(ctx).status(), false);
        ctx.subscribe_to_model(&owner, |me, _, _, ctx| { me.refresh(ctx); });
        ...
        ctx.subscribe_to_view(&picker, |me, _, _, ctx| {
            ...
            if let Some(view) = ctx.view_with_id::<crate::terminal::TerminalView>(ctx.window_id(), me.terminal_view_id) {
                ctx.focus(&view);
            }
            ctx.notify();
        });
        let mut view = Self { transport, owner, model, status, acknowledge_mouse: Default::default(),
            picker_mouse: Default::default(), retry_mouse: Default::default(), recovery_mouse: Default::default(),
            picker, picker_open: false, terminal_view_id };
    ...
    fn refresh(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let status = status_text(&self.transport.status(), self.owner.as_ref(ctx).status(), running);
        if status != self.status { self.status = status; ctx.notify(); }
    }

    fn schedule_tick(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(async { async_io::Timer::after(Duration::from_millis(250)).await; }, |me, _, ctx| {
            me.refresh(ctx);
...
impl Entity for StatusView { type Event = (); }
...
impl TypedActionView for StatusView {
    ...
    fn handle_action(&mut self, action: &Action, ctx: &mut ViewContext<Self>) {
        match action {
            Action::OpenRecovery => {
                if !self.status.recover { return; }
                if let Some(view) = ctx.view_with_id::<crate::terminal::TerminalView>(ctx.window_id(), self.terminal_view_id)
                    && let Some(attachment) = view.as_ref(ctx).persistent_workspace.clone() {
                    view.update(ctx, |_, ctx| ctx.emit(crate::terminal::view::Event::OpenPersistentRecovery(attachment)));
                }
            }
            ...
            Action::OpenWorkspaces => {
                let attachment = ctx.view_with_id::<crate::terminal::TerminalView>(ctx.window_id(), self.terminal_view_id)
                    .and_then(|view| view.as_ref(ctx).persistent_workspace.clone());
                let connection = attachment.map(|attachment| PickerConnection {
                    connection: attachment.connection, control_path: attachment.owner.as_ref(ctx).socket_path(),
                ...
                self.picker.update(ctx, |picker, ctx| picker.open(connection, ctx));
                ctx.notify();
            }
        }
    }
...
impl View for StatusView {
    fn ui_name() -> &'static str { "PersistentWorkspaceStatus" }
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let switcher = Hoverable::new(self.picker_mouse.clone(), move |_| {
            Container::new(Text::new("Workspaces", font, size).with_color(theme.ansi_fg_blue()).finish())
                .with_horizontal_padding(12.).finish()
        }).on_click(|ctx, _, _| ctx.dispatch_typed_action(Action::OpenWorkspaces))
            .with_cursor(Cursor::PointingHand).finish();
        let mut column = Flex::column().with_child(Flex::row()
            .with_child(Text::new(self.status.message.clone(), font, size)
                .with_color(foreground).soft_wrap(true).finish())
            .with_child(switcher).finish());
        if self.status.acknowledge {
            column.add_child(Hoverable::new(self.acknowledge_mouse.clone(), move |_| {
                Container::new(Text::new("Resume input without resending", font, size)
                    .with_color(theme.ansi_fg_yellow()).finish()).with_vertical_padding(5.).finish()
            }).on_click(|ctx, _, _| ctx.dispatch_typed_action(Action::AcknowledgeInput))
                .with_cursor(Cursor::PointingHand).finish());
        }
        if self.status.retry {
            column.add_child(Hoverable::new(self.retry_mouse.clone(), move |_| {
                Container::new(Text::new("Retry output (never resends input)", font, size)
                    .with_color(theme.ansi_fg_blue()).finish()).with_vertical_padding(5.).finish()
        ...
        if self.status.recover {
            column.add_child(Text::new("Missing output cannot be reconstructed. Recovery opens the live tmux screen in a separate pane; it does not restart the job or restore missing blocks.", font, size)
                .with_color(foreground).soft_wrap(true).finish());
            column.add_child(Hoverable::new(self.recovery_mouse.clone(), move |_| {
                Container::new(Text::new("Open live recovery terminal", font, size)
                    .with_color(theme.ansi_fg_blue()).finish()).with_vertical_padding(5.).finish()
            }).on_click(|ctx, _, _| ctx.dispatch_typed_action(Action::OpenRecovery))
                .with_cursor(Cursor::PointingHand).finish());
        }
        let mut stack = Stack::new().with_child(Container::new(column.finish()).with_background(background)
            .with_horizontal_padding(12.).with_vertical_padding(6.).finish());
        if self.picker_open {
            stack.add_positioned_overlay_child(ChildView::new(&self.picker).finish(),
                OffsetPositioning::offset_from_parent(vec2f(0., -4.), ParentOffsetBounds::WindowByPosition,
                    ParentAnchor::TopLeft, ChildAnchor::BottomLeft));
        }
        stack.finish()
    }
}
...
mod tests {
    ...
    fn persistent_status_never_reports_a_disconnected_job_as_idle() {
        ...
        let text = status_text(&handle.status(), &ConnectionStatus::Reconnecting {
            attempt: 2, error: "Network unavailable".into() }, false);
    ...
    fn persistent_expired_history_does_not_offer_a_misleading_read_retry() {
        let (handle, _) = TransportHandle::channel();
        let mut status = handle.status();
        status.phase = TransportPhase::HistoryGap { earliest_cursor: 1048576 };
        let text = status_text(&status, &ConnectionStatus::Connected, true);
