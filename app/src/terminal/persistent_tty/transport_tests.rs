...
fn persistent_input_is_rejected_without_destroying_the_terminal() {
    ...
    for phase in [
        ...
        TransportPhase::Exited,
        TransportPhase::HistoryGap {
            earliest_cursor: 1024,
        },
        TransportPhase::Failed("recorder stopped".into()),
    ] {
        handle.update(3, phase);
        assert!(matches!(
            handle.send(Message::Input(vec![3].into())),
            Err(EventLoopSendError::Other(_))
        ));
    }
}
...
fn persistent_input_keeps_its_connection_epoch() {
    ...
    handle
        .send(Message::Input(b"run-once\r".to_vec().into()))
        .unwrap();
...
fn persistent_shutdown_pauses_for_undo_close_without_sending_remote_input() {
    ...
    assert!(
        handle
            .send(Message::Input(b"exit\r".to_vec().into()))
            .is_err()
    );
    handle.resume();
...
fn persistent_reopen_invalidates_pending_input_even_on_the_same_connection() {
    ...
    handle.update(1, TransportPhase::Live);
    handle
        .send(Message::Input(b"do-not-retry\r".to_vec().into()))
        .unwrap();
    handle.pause();
    handle.status.lock().input_delivery_unknown = true;
    handle.resume();
...
fn persistent_recorder_loss_is_a_failure_not_an_endless_attach() {
    for phase in [ReplayPhase::Disconnected, ReplayPhase::Closed] {
        assert!(matches!(
            phase_after_replay(phase, true, false),
            TransportPhase::Failed(_)
        ));
    }
    assert_eq!(
        phase_after_replay(ReplayPhase::Closed, true, true),
        TransportPhase::Exited
    );
    assert_eq!(
        phase_after_replay(ReplayPhase::Live, true, true),
        TransportPhase::Replaying
    );
    assert_eq!(
        phase_after_replay(ReplayPhase::Replaying, true, true),
        TransportPhase::Replaying
    );
}
...
fn persistent_shell_exit_waits_for_the_recorders_final_page() {
    for shell_ready in [false, true] {
        for phase in [ReplayPhase::Replaying, ReplayPhase::Live] {
            assert_eq!(
                phase_after_replay(phase, shell_ready, true),
                TransportPhase::Replaying
            );
        }
        assert_eq!(
            phase_after_replay(ReplayPhase::Closed, shell_ready, true),
            TransportPhase::Exited
        );
    }
}
...
fn persistent_history_gap_retains_its_identity_and_never_enables_input() {
    let phase = phase_after_replay(
        ReplayPhase::HistoryGap {
            earliest_cursor: 4096,
        },
        true,
        false,
    );
