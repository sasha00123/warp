...
impl Entity for ConnectionOwner {
    type Event = ();
}
...
impl ConnectionOwner {
    pub fn create(recipe: SshRecipe, ctx: &mut AppContext) -> Result<ModelHandle<Self>> {
        ...
        Ok(ctx.add_model(|ctx: &mut ModelContext<Self>| {
            ...
            ctx.subscribe_to_model(&manager, |me, _, event, ctx| match event {
    ...
    pub fn socket_path(&self) -> PathBuf {
        ...
        self.directory
            .path()
            .join(format!("{:016x}", self.session_id.as_u64()))
    }
            ...
            |me, _, ctx| {
                    ...
                    .is_some_and(|deadline| Instant::now() >= deadline)
                {
                    ...
                    let dead = me
                        .master
                        .as_mut()
                        .is_none_or(|master| !matches!(master.try_status(), Ok(None)));
            ...
            move |me, result: Result<Child>, ctx| {
                ...
                match result {
                    Ok(child) => {
                        ...
                        let crash_reporting = PrivacySettings::handle(ctx)
                            .as_ref(ctx)
                        ...
                        RemoteServerManager::handle(ctx).update(ctx, |manager, ctx| {
                            manager.connect_session(
                                ...
                                transport,
                                auth,
                                Some("Persistent SSH workspace".into()),
                                ctx,
                            );
                        });
                    }
...
async fn wait_for_master(child: &mut Child, socket: &Path, tail: &Mutex<Vec<u8>>) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Err(anyhow!("Timed out establishing a private SSH connection"));
        }
