...
impl Lab {
    ...
    fn consume_until(
        ...
        cursor: &mut u64,
        ready: impl Fn(&TerminalModel) -> bool,
    ) {
...
fn verify_native_shell(shell: ShellType) {
    ...
    let session_id = SessionId::from(
        remote_server::persistent_shell::session_id(&workspace.generation).unwrap(),
    );
    let new_model = || {
    ...
    assert_eq!(
        remote_server::persistent_shell::activity(&lab.root, &workspace.id, &workspace.generation),
        Some("idle")
    );
    ...
    lab.backend
        .send_input(&workspace.id, &workspace.generation, command)
        .unwrap();
    lab.consume_until(&workspace, &mut model, &mut parser, &mut cursor, |model| {
...
fn persistent_native_shell_exit_finishes_its_block_without_closing_the_view() {
    ...
    lab.backend
        .send_input(&workspace.id, &workspace.generation, b"exit 7\r")
        .unwrap();
    lab.consume_until(&workspace, &mut model, &mut parser, &mut cursor, |model| {
        model
            .block_list()
    ...
    let exit_code = loop {
        let current = lab
            ...
            .resolve(&workspace.id, &workspace.generation)
            .unwrap();
        if let Some(code) = current.exit_code {
            assert!(current.exited);
            break code;
        }
        assert!(
            Instant::now() < deadline,
            "tmux did not report shell exit status"
        );
    ...
    assert!(
        !model.is_read_only(),
        "Finishing a retained block must not dispose its terminal view"
    );
}
