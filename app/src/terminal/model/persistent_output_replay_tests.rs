...
use crate::terminal::model::ansi::{
    CommandFinishedValue, CompletionMetadata, Handler, PrecmdValue, PreexecValue, PromptMetadata,
};
...
fn persistent_output_replay_reconstructs_running_command_without_local_input() {
    for recovery in [false, true] {
        ...
        terminal.preexec(PreexecValue {
            command: "while true; do date; sleep 15; done".into(),
            session_id: None,
        });
        assert_eq!(
...
fn persistent_output_replay_completion_then_another_command_preserves_block_boundary() {
    ...
    assert_eq!(
        terminal.block_list().previous_command_exit_code(),
        Some(ExitCode::from(7))
    );
    terminal.preexec(PreexecValue {
        command: "second still running".into(),
        session_id: None,
    });
    assert_eq!(
        terminal.block_list().active_block().state(),
...
fn persistent_output_replay_keeps_existing_command_text_and_rejects_repeated_preexec() {
    ...
    terminal
        .block_list_mut()
        .active_block_mut()
        .init_command(b"my-alias");
    terminal.preexec(PreexecValue {
        command: "expanded-command".into(),
        session_id: None,
    });
    assert_eq!(
        terminal.block_list().active_block().command_to_string(),
    ...
    terminal.preexec(PreexecValue {
        command: "late stale command".into(),
        session_id: None,
    });
    assert_eq!(
        terminal.block_list().active_block().command_to_string(),
...
fn persistent_output_replay_fallback_is_not_enabled_for_ordinary_terminals() {
    let mut terminal = TerminalModel::mock(None, None);
    terminal.preexec(PreexecValue {
        command: "not-local-editor-text".into(),
        session_id: None,
    });
...
fn persistent_output_replay_multiline_commands_start_at_the_original_column() {
    for newline in ["\n", "\r\n"] {
        for recovery in [false, true] {
            ...
            terminal.enable_persistent_workspace_mode();
            terminal.preexec(PreexecValue {
                command: format!("printf first{newline}printf second"),
                session_id: None,
            });
            assert_eq!(
                terminal.block_list().active_block().command_to_string(),
