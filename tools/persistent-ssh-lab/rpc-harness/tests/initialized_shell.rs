...
use persistent_workspace_rpc_harness::persistent_workspace::{
    Backend, BootstrapProfile, Workspace,
};
...
impl Lab {
    ...
    fn create(&self, shell: &str) -> Workspace {
        self.backend
            .create_initialized(
                ...
                None,
                profile(shell),
                Path::new(env!("CARGO_BIN_EXE_recorder")),
                &self.root,
            )
            .unwrap()
    }
    fn output(&self, workspace: &Workspace, marker: &str) -> String {
        ...
        loop {
            let page = self
                .backend
                .read_output(&workspace.id, &workspace.generation, &self.root, 0)
                .unwrap();
...
fn profile(shell: &str) -> BootstrapProfile<'_> {
    BootstrapProfile {
        shell,
        init_script:
            br#"WARP_SESSION_ID=@@WARP_SESSION_ID@@; printf 'INIT:%s\n' "$WARP_SESSION_ID""#,
        bootstrap_script:
            b"export EW_STARTUP_VALUE=preserved; stty -echo; PS1=''; printf 'READY\\n'\n",
    }
}
...
fn startup_is_recorded_and_create_retry_does_not_bootstrap_a_running_job() {
    ...
    assert!(initial.contains(&format!(
        "INIT:{}",
        persistent_shell::session_id(&workspace.generation).unwrap()
    )));
    lab.backend
        .send_input(
            &workspace.id,
            &workspace.generation,
            b"while :; do printf 'JOB:%s\\n' \"$EW_STARTUP_VALUE\"; sleep 0.1; done\r",
        )
        .unwrap();
    ...
    lab.backend
        .send_input(&workspace.id, &workspace.generation, &[3])
        .unwrap();
}
...
fn retention_survives_create_retry_and_history_deletes_only_after_termination() {
    ...
    for (policy, version) in [
        (RetentionPolicy::UntilWorkspaceDeleted, "EWJ2"),
        (RetentionPolicy::Rolling, "EWJ1"),
    ] {
        let lab = Lab::new();
        let id = "a".repeat(32);
        let workspace = lab
            .backend
            .create_initialized_with_retention(
                &id,
                None,
                profile("bash"),
                Path::new(env!("CARGO_BIN_EXE_recorder")),
        ...
        assert!(
            std::fs::read_to_string(journal.join("manifest"))
                .unwrap()
                .starts_with(version)
        );
        ...
        assert!(
            lab.backend
                .delete_retained_history(&id, &workspace.generation, &lab.root)
                .is_err()
        );
        ...
        assert!(
            !lab.root
                .join(format!("{id}-{}.shell", workspace.generation))
                .exists()
        );
        lab.backend
            .delete_retained_history(&id, &workspace.generation, &lab.root)
